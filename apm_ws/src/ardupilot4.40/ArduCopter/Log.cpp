...
struct PACKED log_stab_to_loit {
    LOG_PACKET_HEADER;
    uint64_t time_us;
    float    throttle;
    int16_t  rngfinder_cm;
    int16_t  z_speed;
    uint8_t  in_air_status;
    uint8_t  ground_status;
    uint8_t  judge_status;
};


void Copter::Log_Write_Stab_to_loit_msg(float thr,int16_t rngfinder,int16_t z_spd,uint8_t air_status,uint8_t ground_status,uint8_t ju_sta)
{
    const log_stab_to_loit pkt {
        LOG_PACKET_HEADER_INIT(LOG_STAB_TO_LOIT_MSG),
        time_us         : AP_HAL::micros64(),
        throttle        : thr,
        rngfinder_cm    : rngfinder,
        z_speed         : z_spd,
        in_air_status   : air_status,
...
// @Field: ClimbRt: Climb rate
    ...
    { LOG_GUIDED_ATTITUDE_TARGET_MSG, sizeof(log_Guided_Attitude_Target),
      ...
      { LOG_STAB_TO_LOIT_MSG, sizeof(log_stab_to_loit),
      "STAB",  "QfhhBBB",    "TimeUS,thr,rng,up,air_s,ground_s,ju_sta", "-------", "-------" , true },
};

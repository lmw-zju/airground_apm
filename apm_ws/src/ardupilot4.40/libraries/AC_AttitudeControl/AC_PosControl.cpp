...
void AC_PosControl::set_ground_air_status(uint8_t status)
{
   float temp_pos_p,temp_vel_p,temp_vel_i;
   get_pos_xy_p().load_gains_kp();//load default value
   get_vel_xy_pid().load_gains_kp();
   get_vel_xy_pid().load_gains_ki();
   
   temp_pos_p = get_pos_xy_p().kP();
   temp_vel_p = get_vel_xy_pid().kP();
   temp_vel_i = get_vel_xy_pid().kI();
   if(!status)//on ground ,magnify data p i
   {    
       get_pos_xy_p().kP(temp_pos_p*2.0);
       get_vel_xy_pid().kP(temp_vel_p*2.0);
       get_vel_xy_pid().kI(temp_vel_i*2.0);
      gcs().send_text(MAV_SEVERITY_CRITICAL, "ground:pos_p%0.2fvel_p:%0.2f %0.2f",(float)get_pos_xy_p().kP(),(float)get_vel_xy_pid().kP(),(float)get_vel_xy_pid().kI());
   }
   else//normal data
   {
      get_pos_xy_p().kP(temp_pos_p);
      get_vel_xy_pid().kP(temp_vel_p);
      get_vel_xy_pid().kI(temp_vel_i);
      gcs().send_text(MAV_SEVERITY_CRITICAL, "air:pos_p%0.2fvel_p:%0.2f %0.2f",(float)get_pos_xy_p().kP(),(float)get_vel_xy_pid().kP(),(float)get_vel_xy_pid().kI());
   }
